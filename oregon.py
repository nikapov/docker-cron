...
localhost = '127.0.0.1'
inTempTopic = 'oregon/inTemp'
inTempMaxTopic = 'oregon/inTempMax'
inTempMinTopic = 'oregon/inTempMin'
outTempTopic = 'oregon/outTemp'
outTempMaxTopic = 'oregon/outTempMax'
outTempMinTopic = 'oregon/outTempMin'
inHumTopic = 'oregon/inHum'
inHumMaxTopic = 'oregon/inHumMax'
inHumMinTopic = 'oregon/inHumMin'
outHumTopic = 'oregon/outHum'
outHumMaxTopic = 'oregon/outHumMax'
outHumMinTopic = 'oregon/outHumMin'

def publishData(mqttc,topic,data):
      mqttc.connect(localhost,1883,60)
      mqttc.publish(topic,data)
      mqttc.disconnect()
        
WEATHERSTATION_NAME = "IDTW213P" # IDTW213R for RAR218HG
...
class WeatherStation:
	...
	def getIndoorHum(self):
		if 'index0_humidity' in self._data:
			...
			logging.debug('Indoor hum : %.1f%%, max : %.1f%%, min : %.1f%%', hum, max, min)
			return hum,max,min
		else:
			return None,None,None
	
	def getOutdoorHum(self):
		if 'index1_humidity' in self._data:
			...
			logging.debug('Outdoor hum : %.1f%%, max : %.1f%%, min : %.1f%%', hum, max, min)
			return hum,max,min
		else:
			return None,None,None
	...
	if weatherStationMacAddr is None:
		...
		try:
			...
			if weatherStation.monitorWeatherStation() is not None:
				...
				indoor,inMax,inMin = weatherStation.getIndoorTemp()
				outdoor,outMax,outMin = weatherStation.getOutdoorTemp()
				indoor_hum,inMaxHum,inMinHum = weatherStation.getIndoorHum()
				outdoor_hum,outMaxHum,outMinHum = weatherStation.getOutdoorHum()
                                mqttc=mqtt.Client()
                                publishData(mqttc,inTempTopic,indoor)
                                publishData(mqttc,inTempMaxTopic,inMax)
                                publishData(mqttc,inTempMinTopic,inMin)
                                publishData(mqttc,outTempTopic,outdoor)
                                publishData(mqttc,outTempMaxTopic,outMax)
                                publishData(mqttc,outTempMinTopic,outMin)
                                publishData(mqttc,inHumTopic,indoor_hum)
                                publishData(mqttc,inHumMaxTopic,inMaxHum)
                                publishData(mqttc,inHumMinTopic,inMinHum)
                                publishData(mqttc,outHumTopic,outdoor_hum)
                                publishData(mqttc,outHumMaxTopic,outMaxHum)
                                publishData(mqttc,outHumMinTopic,outMinHum)
			else:
